...
def main():
    ...
    try:
        ...
        for url_ in range(len(moveList['id'])):
            ...
            s_url_ = getSoup(review_url,cookie);  # 解析html页面
            ...
            if len(info_s) == 5:
                for info_db in range(len(info_s)):
                    ...
                    sql = "insert into tb_douban_movie(origin_movie_id,type,name,release_year,image,director,actor,genre,location,language,date_published," \
                          "duration,alias,rating_value,description,episode,imdb,status,create_time,author,tags) " \
                          ...
                          info_s[info_db]['status'] + "'" + "," + "'" + info_s[info_db]['create_time'] + "'" + "," + "'" + info_s[info_db]['author'] + "'"+ "," + "'" + info_s[info_db]['tag'] + "')";
                    # print "---"+sql+"---"
